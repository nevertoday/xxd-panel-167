...
# XXD Panel 167｜Centred Wave-form Raised-ink Relief
...
Eight first-pass Panel 167 samples generated independently from different source images, with short English copy. Landscape: 1536×864 with equal left/right halves; portrait: 1152×1536 with equal top/bottom halves.

Reviewed for source correspondence, wave-based composition and raised-ink relief. The car in 05 remains realistic and metallic, 06 is detailed, and 09/12 retain extensive scenery. Many waves reach the edges instead of leaving generous whitespace all around; some colors are muted. These actual first-pass results have not all passed style review.
...
Use a centred abstract rolling wave or curved colour form as the compositional carrier; it need not depict seawater. Integrate the recognisable subject into its peaks or turns. Render raised screen-print ink with restrained relief edges, layered thickness, ink grain and very slight side light. Keep 2–4 fresh source colours and vast whitespace; avoid plastic, metallic CG or a literal complete scene.
...
- `design-only`: the full canvas contains only Panel 167's designed translation; the photograph remains a non-visible reference.
...
```bash
git clone https://github.com/nevertoday/xxd-panel-167.git
npx skills add https://github.com/nevertoday/xxd-panel-167 --skill xxd-panel-167
```

Restart the agent session after installation, then invoke `$xxd-panel-167`. Add `--global --agent codex --yes` when a user-level Codex installation is wanted.

Common examples:

```text
/xxd-panel-167 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale en-US
/xxd-panel-167 photo.jpg --mode left-right --size 16:9 --text prompt --locale en-US
/xxd-panel-167 photo.jpg --mode design-only --size 9:16 --text none
/xxd-panel-167 ./photos --mode design-only --size auto,3:4 --text prompt --locale ja-JP
```

See [SKILL.md](SKILL.md) for the full runtime contract and the [English](references/xxd-panel-167-prompt.en.md) or [Chinese](references/xxd-panel-167-prompt.zh-CN.md) runtime adapter.
...
Input image for this panel: `assets/inputs/input-09`

![Re-selected input](assets/inputs/input-09.jpg)
