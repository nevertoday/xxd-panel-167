...
# XXD Panel 167｜중앙 파형과 두꺼운 잉크 부조
...
서로 다른 원본에서 독립 생성한 Panel 167의 첫 시도 샘플 8장입니다. 짧은 영어 문구를 사용합니다. 가로는 1536×864 좌우 균등 분할, 세로는 1152×1536 상하 균등 분할입니다.

원본 대응 관계, 파도형 구도와 두꺼운 잉크 부조를 검토했습니다. 05의 자동차는 사실적이고 금속성이 강하며 06은 세부가 많고 09·12는 풍경을 많이 남겼습니다. 여러 파형이 가장자리까지 이어져 사방의 넓은 여백이 부족하며 일부 색은 탁합니다. 모든 스타일 기준을 통과한 것은 아닌 실제 첫 결과입니다.
...
중앙의 추상 파형이 피사체를 받칩니다. 실제 바닷물일 필요는 없습니다. 두꺼운 잉크 입자와 약한 측광, 큰 여백을 씁니다.
...
- `design-only`: 전체 캔버스에 Panel 167의 디자인 번역만 표시하고 사진은 보이지 않는 참고 자료로 사용합니다.
...
설치 후 Agent 세션을 다시 시작하고 `$xxd-panel-167`을 호출하세요. 사용자 단위 Codex 설치에는 `--global --agent codex --yes`를 추가할 수 있습니다.

```text
/xxd-panel-167 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ko-KR
...
전체 실행 계약은 [SKILL.md](SKILL.md), 런타임 어댑터는 [영어](references/xxd-panel-167-prompt.en.md)와 [중국어](references/xxd-panel-167-prompt.zh-CN.md)를 확인하세요.
